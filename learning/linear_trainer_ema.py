...
class LinearTrainer(BaseTrainer):
    ...
    def wrap_up(self, model, classifier, model_ema=False):
        ...
        if model_ema:
            ...
            model_ema.eval()
            model_ema = DDP(model_ema, device_ids=[args.gpu])
            return model, classifier, model_ema
        else:
    ...
    def load_encoder_weights_all(self, model, model_ema=False):
        """load pre-trained weights for encoder
        ...
        if args.ckpt:
            ...
            if args.modal == 'RGB':
                ...
                if model_ema:
                    state_dict_ema = ckpt['model_ema']
                    encoder_state_dict = OrderedDict()
                    for k, v in state_dict_ema.items():
                        k = k.replace('module.', '')
                        encoder_state_dict[k] = v
                    model_ema.load_state_dict(encoder_state_dict)
    ...
    def train(self, epoch, train_loader, model, classifier,
              criterion, optimizer, model_ema=False):
        time1 = time.time()
        args = self.args
        ...
        for idx, data in enumerate(train_loader):
            ...
            with torch.no_grad():
                if args.keephead == 'jigsaw':
                    ...
                    feat = feat.detach()
                if model_ema:
                    feat_ema = model_ema(x=input, mode=1)
                    feat_ema = feat_ema.detach()
                    feat = torch.cat((feat, feat_ema), dim=1)
    ...
    def validate(self, epoch, val_loader, model, classifier, criterion, model_ema=False):
        time1 = time.time()
        args = self.args
    ...
    def test(self, epoch, val_loader, model, classifier, criterion, model_ema=False):
        time1 = time.time()
        args = self.args
            ...
            for idx, data in enumerate(val_loader):
                ...
                with torch.no_grad():
                    if args.keephead == 'jigsaw':
                        ...
                        feat = feat.detach()
                        if model_ema:
                            feat_ema = model_ema(x=input, mode=2)
                            feat_ema = feat_ema.detach()
                            feat = torch.cat((feat, feat_ema), dim=1)
                output = classifier(feat)
                loss = criterion(output, target)
