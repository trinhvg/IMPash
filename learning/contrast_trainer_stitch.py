...
class ContrastTrainer(BaseTrainer):
    ...
    def train(self, epoch, train_loader, model, model_ema, contrast,
        ...
        time1 = time.time()
        if args.method == 'PatchSMoco':
            outs = self._train_moco_PIRL(epoch, train_loader, model, model_ema,
    ...
    @staticmethod
    def _gather_x(x, local_gp):
        node_x = [torch.ones_like(x) for _ in range(dist.get_world_size(local_gp))]
        dist.all_gather(node_x, x.contiguous(), group=local_gp, async_op=False)
        return torch.cat(node_x, dim=0)

    def _shuffle_bn_jig(self, x, x_jig, model_ema):
        """ Shuffle BN implementation

        Args:
          x: input image on each GPU/process
          x_jig: input jigsaw image on each GPU/process
          model_ema: momentum encoder on each GPU/process
        """
        args = self.args
        local_gp = self.local_group
        bsz = x.size(0)
        # print('x_jig_input' ,x_jig.shape)

        _, c, h, w = x_jig.size()
        x_jig = x_jig.view(bsz, 9, c, h, w)
        ...
        #
        # # gather x_jig locally for each node
        # node_x = [torch.ones_like(x) for _ in range(dist.get_world_size(local_gp))]
        # dist.all_gather(node_x, x.contiguous(), group=local_gp, async_op=False)
        # node_x = torch.cat(node_x, dim=0)

        node_x = self._gather_x(x, local_gp)
        node_x_jig = self._gather_x(x_jig, local_gp)

        # shuffle bn
        shuffle_ids = torch.randperm(bsz * dist.get_world_size(local_gp)).cuda()
        ...
        with torch.no_grad():
            ...
            this_x_jig = node_x_jig[this_ids]

            this_x_jig = this_x_jig.view(bsz * 9, c, h, w)
            # print(this_x_jig.shape)
            if args.jigsaw_ema:
                k, k_jig = model_ema(this_x, x_jig=this_x_jig, mode=0)
            if args.jigsaw:
        ...
        all_k_jig = self._global_gather(k_jig)

        # unshuffle bn
        node_id = args.node_rank
        ngpus = args.ngpus_per_node
        node_k = all_k[node_id*ngpus*bsz:(node_id+1)*ngpus*bsz]
        node_k_jig = all_k_jig[node_id*ngpus*bsz:(node_id+1)*ngpus*bsz]

        this_ids = reverse_ids[args.local_rank*bsz:(args.local_rank+1)*bsz]
        k = node_k[this_ids]
        k_jig = node_k_jig[this_ids]

        return k, all_k, k_jig, all_k_jig


    def _shuffle_bn_jigstitch(self, x, x_jig, model_ema):
        """ Shuffle BN implementation
    ...
    def _shuffle_bn(self, x, model_ema):
        ...
        with torch.no_grad():
            this_x = node_x[this_ids]
            if args.jigsaw:
                k = model_ema(this_x, x_jig=this_x, mode=1)
            else:
                k = model_ema(this_x, mode=1)
    ...
    def _train_moco(self, epoch, train_loader, model, model_ema, contrast,
        ...
        for idx, data in enumerate(train_loader):
            ...
            inputs = data[0].float().cuda(args.gpu, non_blocking=True) # torch.Size([128, 6, 224, 224])
            bsz = inputs.size(0)
            ...
            if not args.jigsaw_ema:
                # shuffle BN for momentum encoder
                k, all_k = self._shuffle_bn(x2, model_ema)

            # print(k.shape, all_k.shape)
            # torch.Size([128, 128]), torch.Size([256, 128])


            # loss and metrics
            if args.jigsaw_ema:
                if args.jigsaw_stitch:
                    inputs_jig = data[2].float().cuda(args.gpu, non_blocking=True)
                    x_j1, x_j2 = torch.split(inputs_jig, [3, 3], dim=1)
                    # bsz, m, c, h, w = x_j1.shape
                    # x_j1 = x_j1.reshape(bsz * m, c, h, w)
                    # x_j2 = x_j2.reshape(bsz * m, c, h, w)
                    q, q_jig = model(x1, x_j1)
                    # shuffle BN for momentum encoder
                    k, all_k, k_jig, all_k_jig = self._shuffle_bn_jigstitch(x=x2, x_jig=x_j2, model_ema=model_ema)
                    output = contrast(q, k, q_jig, k_jig, all_k, all_k_jig)
                    ...
                    inputs_jig = data[2].float().cuda(args.gpu, non_blocking=True)
                    x_j1, x_j2 = torch.split(inputs_jig, [9, 9], dim=1)
                    bsz, m, c, h, w = x_j1.shape
                    x_j1 = x_j1.reshape(bsz * m, c, h, w)
                    x_j2 = x_j2.reshape(bsz * m, c, h, w)
                    ...
                    #     img = x_j2[i + 8, :].cpu().numpy().transpose(1, 2, 0)
                    ...
                    # shuffle BN for momentum encoder
                    k, all_k, k_jig, all_k_jig = self._shuffle_bn_jig(x=x2, x_jig=x_j2, model_ema=model_ema)
                    output = contrast(q, k, q_jig, k_jig, all_k, all_k_jig)

                    if args.jig_version == 'V0':
                        losses, accuracies = self._compute_loss_accuracy(
                            logits=output[:-1], target=output[-1],
                            criterion=criterion)
                        loss = (1 - args.beta) * (losses[0] + losses[1]) + \
                        ...
                        update_loss_jig = 0.5 * (losses[2] + losses[3])
                        update_acc_jig = 0.5 * (accuracies[2] + accuracies[3])
                    elif args.jig_version == 'V1':
                        losses, accuracies = self._compute_loss_accuracy(
                            logits=output[:-1], target=output[-1],
                            criterion=criterion)
                        loss = (1 - args.beta) * (losses[0]) + \
                            args.beta * (losses[1] + losses[2])
                        update_loss =  (losses[0])
                        update_acc = (accuracies[0])
                        update_loss_jig = 0.5 * (losses[1] + losses[2])
                        update_acc_jig = 0.5 * (accuracies[1] + accuracies[2])
                    else:
                        losses, accuracies = self._compute_loss_accuracy(
                            logits=output[:-1], target=output[-1],
                            criterion=criterion)
                        loss = (1 - args.beta) * losses[0] + \
                            args.beta * losses[1]
                        update_loss = losses[0] #tensor(5.1794, device='cuda:1', grad_fn=<NllLossBackward>)
                        update_acc = accuracies[0] #100.0
                        update_loss_jig = losses[1]
                        update_acc_jig = accuracies[1]

            elif args.jigsaw_stitch:
                inputs_jig = data[2].float().cuda(args.gpu, non_blocking=True)
    ...
    def _train_moco_PIRL(self, epoch, train_loader, model, model_ema, contrast,
                    criterion, optimizer):
        """
        MoCo encoder style training. This needs two forward passes,
        one for normal encoder, and one for moco encoder
        ...
        for idx, data in enumerate(train_loader):
            ...
            # shuffle BN for momentum encoder
            k, all_k = self._shuffle_bn(inputs_jig, model_ema)
            q = model(inputs)
