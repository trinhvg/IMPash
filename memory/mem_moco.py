...
class RGBMoCoJig(BaseMoCo):
    """Single Modal (e.g., RGB) MoCo-style cache"""
    def __init__(self, n_dim, K=65536, T=0.07):
        super(RGBMoCoJig, self).__init__(K, T)
        # create memory queue
    ...
    def forward(self, q, k, q_jig=None, k_jig=None, all_k=None, all_k_jig=None):
        """
        Args:
          q: query on current node
          k: key on current node
        ...
        queue = self.memory_1.clone().detach()
        queue_jig = self.memory_2.clone().detach()
        logits = self._compute_logit(q, k, queue)
        logits2 = self._compute_logit(q, k_jig, queue_jig)
        if q_jig is not None:
            logits1_jig = self._compute_logit(q_jig, k, queue)
            logits2_jig = self._compute_logit(q_jig, k_jig, queue_jig)
        ...
        # update memory
        all_k = all_k if all_k is not None else k
        all_k_jig = all_k_jig if all_k_jig is not None else k_jig
        self._update_memory(all_k, self.memory_1)
        self._update_memory(all_k_jig, self.memory_2)
        self._update_pointer(all_k.size(0))

        if q_jig is not None:
            return logits,  logits2, logits1_jig, logits2_jig, labels
        else:
            return logits, labels


class RGBMoCoJig_v1(BaseMoCo):
    """Single Modal (e.g., RGB) MoCo-style cache"""
    def __init__(self, n_dim, K=65536, T=0.07):
        super(RGBMoCoJig_v1, self).__init__(K, T)
        # create memory queue
        self.register_buffer('memory_1', torch.randn(K, n_dim))
        self.register_buffer('memory_2', torch.randn(K, n_dim))
        self.memory_1 = F.normalize(self.memory_1)
    ...
    def forward(self, q, k, q_jig=None, k_jig=None, all_k=None, all_k_jig=None):
        ...
        if q_jig is not None:
            return logits, logits2,  logits_jig, labels
        else:
            return logits, labels
